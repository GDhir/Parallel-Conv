...
__global__ void cnn(int N,int C,int K,int H,int W,int R, int S, int u, int v, int P, int Q,
    ...
    if( idx < N*K*P*Q/16 ) {
        ...
        for(unsigned int c=0; c<C; c++) { // input feature map
            for (unsigned int r = 0; r<R; r++) { // filter height
                for (unsigned int s = 0; s < S; s++) {// filter width
                    ...
                    sumval9 += d_input[n*C*H*W + c*H*W + (ij+r)*W + ii+s] * d_weight[(k + 8)*C*R*S+c*R*S+r*S+s];
                    sumval10 += d_input[n*C*H*W + c*H*W + (ij+r)*W + ii+s] * d_weight[(k + 9)*C*R*S+c*R*S+r*S+s];
                    sumval11 += d_input[n*C*H*W + c*H*W + (ij+r)*W + ii+s] * d_weight[(k + 10)*C*R*S+c*R*S+r*S+s];
                    sumval12 += d_input[n*C*H*W + c*H*W + (ij+r)*W + ii+s] * d_weight[(k + 11)*C*R*S+c*R*S+r*S+s];
                    sumval13 += d_input[n*C*H*W + c*H*W + (ij+r)*W + ii+s] * d_weight[(k + 12)*C*R*S+c*R*S+r*S+s];
                    sumval14 += d_input[n*C*H*W + c*H*W + (ij+r)*W + ii+s] * d_weight[(k + 13)*C*R*S+c*R*S+r*S+s];
                    sumval15 += d_input[n*C*H*W + c*H*W + (ij+r)*W + ii+s] * d_weight[(k + 14)*C*R*S+c*R*S+r*S+s];
                    sumval16 += d_input[n*C*H*W + c*H*W + (ij+r)*W + ii+s] * d_weight[(k + 15)*C*R*S+c*R*S+r*S+s];
                }
            }
        }
        ...
        d_output[n*K*P*Q + (k + 8)*P*Q + p*Q + q] = sumval9;
        d_output[n*K*P*Q + (k + 9)*P*Q + p*Q + q] = sumval10;
        d_output[n*K*P*Q + (k + 10)*P*Q + p*Q + q] = sumval11;
        d_output[n*K*P*Q + (k + 11)*P*Q + p*Q + q] = sumval12;
        d_output[n*K*P*Q + (k + 12)*P*Q + p*Q + q] = sumval13;
        d_output[n*K*P*Q + (k + 13)*P*Q + p*Q + q] = sumval14;
        d_output[n*K*P*Q + (k + 14)*P*Q + p*Q + q] = sumval15;
        d_output[n*K*P*Q + (k + 15)*P*Q + p*Q + q] = sumval16;
