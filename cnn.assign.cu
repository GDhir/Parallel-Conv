...
int main(int argc, char *argv[]) {
    ...
    if( N <= 4 ) {
        ...
        Block_Spec.x = BLOCK_SIZE;

        if( C < 256  ) {
            gsize = ceil( N*K*P*Q/ (double)(8*BLOCK_SIZE) );
        }
        else {
        ...
        Block_Spec.x = BLOCK_SIZE;
        gsize = ceil( N*K*P*Q/ (double)(16*BLOCK_SIZE) );
    }
    ...
    cnn<<<gsize, Block_Spec>>>(N,C,K,H,W,R,S,u,v,P,Q,d_input,d_weight,d_output);
